...
#          03-May-2017 HBP embed code within a named namespace to workaround
#                      a ROOT problem.
#$Revision: 1.3 $
#------------------------------------------------------------------------------
import os, sys
...
class NetWriter:
	...
	def __init__(self, name, numnet, ninputs, nhidden, model, writeHeader):
		...
		if self.scaleOutput:
			out.write('  static const double mean[%s_::ninputs+1]={\n' % name)
		else:
			out.write('  static const double mean[%s_::ninputs]={\n' % name)
		delim = ' '
		...
		if self.scaleOutput:
			out.write('  static const double sigma[%s_::ninputs+1]={\n' % name)
		else:
			out.write('  static const double sigma[%s_::ninputs]={\n' % name)
		delim = ' '
		for namen, mean, sigma in self.var:
		...
		out.write('  static const double weight[%s_::nnetworks]'\
                      '[%s_::nparams]={\n' % (name, name))
	...
	def close(self):
		...
		out.write('%sint last=%s_::nnetworks-1)\n' % (tab, name))

		# Write body

		out.write('{\n')
		out.write('  if ( first < 0 ) first = 0;\n')
		out.write('  if ( last  < 0 ) last  = %s_::nnetworks-1;\n' % name)
		out.write('  if ( last  > %s_::nnetworks-1 ) '\
                      'last = %s_::nnetworks-1;\n' % (name, name))
		...
		out.write('\n')
		out.write('  // Compute average over networks\n')
		out.write('  std::vector<double> in(%s_::ninputs);\n' % name)
		for index, (namen, mean, sigma) in enumerate(var[:ninputs]):
			out.write('  in[%d] = (inputs[%d] - %s_::mean[%d]) / '\
                          '%s_::sigma[%d];\n' % \
					  (index, index, name, index, name, index))
		out.write('\n')
		out.write('  int   n = last - first + 1;\n')
		out.write('  double x = 0.0;\n')
		out.write('  for(int i=first; i <= last; i++) '\
				  'x += %s_::netfun(in, %s_::weight[i]);\n' % (name, name))
		out.write('  x /= n;\n')
		if self.scaleOutput:
			out.write('  x = %s_::mean[%s_::ninputs] + '\
                          '%s_::sigma[%s_::ninputs] * x;\n' %\
                          (name, name, name, name))
		out.write('  return x;\n')
		out.write('}\n')
		...
		if len(var) < MAXARGS:
			...
			for namen, mean, sigma in var[:ninputs]:
				...
				out.write('%sdouble %s,\n' % (tab, namen))
				tab = space 
			out.write('%sint first=0, int last=%s_::nnetworks-1)\n' % \
                          (tab,name))
			out.write('{\n')
			out.write('  std::vector<double> in(%s_::ninputs);\n' % name)
			for index, (namen, mean, sigma) in enumerate(var[:ninputs]):
